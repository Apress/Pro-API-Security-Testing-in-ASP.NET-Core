...
public class AccountService : IAccountService
{
    ...
    public async Task<AccountResponseDto?> GetAccountAsync(Guid accountId)
    {
        ...
        return new AccountResponseDto(
            ...
            Status: accountData.Status // Defaulting to Active; adjust as needed
        );
    }
    ...
    {        
        ...
        if (accountUpdateDto.Status.HasValue)
        {
            await _context.Accounts
                .Where(a => a.Id == accountId)
                .ExecuteUpdateAsync(setters => setters
                    .SetProperty(a => a.Status, accountUpdateDto.Status.Value)
                    .SetProperty(a => a.LastModified, DateTime.UtcNow));

            _logger.LogInformation("Bank teller updated account stats for account {AccountId}", 
                accountId);
        }
