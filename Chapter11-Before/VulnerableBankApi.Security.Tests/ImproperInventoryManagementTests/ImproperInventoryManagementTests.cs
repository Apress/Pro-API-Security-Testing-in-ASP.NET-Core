...
public class ImproperInventoryManagementTests(CustomWebApplicationFactory<Program> factory) : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    public async Task Multiple_API_Versions_Should_Not_Be_Active_Simultaneously(string userId, string accountId)
    {
        ...
        var activeVersions = new List<string>();
        ...
        foreach (var endpoint in endpoints)
        {
            ...
            // Always add the response to help with debugging
            activeVersions.Add($"{endpoint} ({response.StatusCode})" + 
                (!response.IsSuccessStatusCode ? $" - {content}" : ""));
        }

        // Assert - Multiple versions should not be active
        activeVersions.Count.Should().BeLessThanOrEqualTo(1, 
            "Multiple API versions are active simultaneously. Found: " + string.Join(", ", activeVersions));
    }
