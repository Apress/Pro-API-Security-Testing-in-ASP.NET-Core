...
builder.Services.AddHttpClient<IExternalVerificationService, ExternalVerificationService>()
    ...
    {
        // VULNERABILITY: No timeout or restrictions
...
// VULNERABLE ENDPOINT: Payment Processor Notification (Most Common in Banking)
externalGroup.MapPost("/notify-payment", async (PaymentNotificationRequest request,
    IExternalVerificationService verificationService) =>
{
    try
    {
        // CRITICAL VULNERABILITY: Accepts any URL for payment notifications
        // This is extremely dangerous in banking context as it can:
        // - Leak transaction data to attackers
        // - Allow SSRF attacks on payment networks
        // - Expose internal payment processing systems
